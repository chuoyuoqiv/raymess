...
if [[ -z $v2ray_id2 ]]; then
    ...
    while :; do
        ...
        "")
            echo
            echo
            echo -e "$yellow V2Ray ID2 = $cyan$v2ray_id2$none"
            echo "----------------------------------------------------------------"
            echo
            break
            ;;
...
if [[ -z $v2ray_port2 ]]; then
    random=$(shuf -i20001-65535 -n1)
    while :; do
        echo -e "请输入 "$yellow"V2Ray"$none" 端口 ["$magenta"1-65535"$none"], 不能选择 "$magenta"80"$none" 或 "$magenta"443"$none" 端口"
        read -p "$(echo -e "(默认端口port: ${cyan}${random}$none):")" v2ray_port2
        [ -z "$v2ray_port2" ] && v2ray_port2=$random
        case $v2ray_port2 in
        80)
            echo
            echo " ...都说了不能选择 80 端口了咯....."
        ...
        [1-9] | [1-9][0-9] | [1-9][0-9][0-9] | [1-9][0-9][0-9][0-9] | [1-5][0-9][0-9][0-9][0-9] | 6[0-4][0-9][0-9][0-9] | 65[0-4][0-9][0-9] | 655[0-3][0-5])
            echo
            echo
            echo -e "$yellow 内部 V2Ray2 端口Internal port = $cyan$v2ray_port2$none"
            echo "----------------------------------------------------------------"
            echo
            break
            ;;
